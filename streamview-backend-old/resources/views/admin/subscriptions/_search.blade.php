<div class="col-xs-12 mb-2  bottom-space">
						...
						<form class="col-sm-8 col-sm-offset-4" action="{{route('admin.subscriptions.index')}}" method="GET" role="search">
						   @csrf
							<div class="flex">
								<input type="text" value="{{Request::get('search_key')??''}}" class="form-control search_input input-space" name="search_key" placeholder="{{tr('subscriptions_search_placeholder')}}">
								...
								<a class="btn btn-default box-left" href="{{route('admin.subscriptions.index')}}">{{tr('clear')}}</a>
