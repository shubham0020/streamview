...
class PayPerView extends Helper {
  ...
  checkReferralInvoice = (event) => {
    ...
    api
      .postMethod("invoice_referral_amount", inputData)
      .then((response) => {
  ...
  handlePromoCode = (event) => {
    ...
    api
      .postMethod("apply/coupon/ppv", inputData)
      .then((response) => {
        if (response.data.success) {
          ToastDemo(
            this.props.toastManager,
  ...
  handlePayment = (event) => {
    ...
    this.setState({transaction_ID:transactionID})
     if(this.state.paymentMode=="COD"){
            this.setState
            ({qrCode: `http://api.qrserver.com/v1/create-qr-code/?&size=10*10&data= userEmail:${this.state.data.email} amount:${this.state.pay_amount} TransID: ${this.state.transaction_ID} paymentMode: ${this.state.paymentMode}`})
  ...
  paymentApiCall = (inputData) => {
    api
      .postMethod("v4/ppv_payment", inputData)
      .then((response) => {
        if (response.data.success) {
...
//console.log(this.state,'--------------')
    ...
    api.getMethod(`payperview?payment_mode=${this.state.paymentMode}&transactionID=${this.state.transaction_ID}&amount=${this.state.pay_amount}&expiry=${new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' +( new Date().getDate()+1) + ' ' +( new Date().toLocaleTimeString())}&video_id=${this.props.location.state.videoDetailsFirst
      .admin_video_id}&type_of_user=${this.props.location.state.videoDetailsFirst
        .type_of_user}&type_of_subscription=${this.props.location.state.videoDetailsFirst
        .type_of_subscription}&userid=${this.state.useDetails.id}&storeid=${this.state.radio}`).then(response => {
  ...
  render() {
    ...
    if (this.state.loadingFirst) {
      ...
      let total = this.state.pay_amount; // same as above, this is the total amount (based on currency) to be paid by using Paypal express checkout

      const client = {
        sandbox: configuration.get("configData.PAYPAL_ID"),
        production: configuration.get("configData.PAYPAL_ID"),
      };
console.log(this.state.data.email,'this')
...
// const date =currDate.setDate(currDate.getDate() + 2);
// console.log(date,'-----------------------')
    ...
    <div style={{width:'160%'}}  className=" modal-content p-4 ">
      ...
      <div className="output-box d-flex flex-wrap border ">
          <div className="col-md-6 p-5">
              <p className="text-dark" >Name : {this.state.data.name}</p>
              <p className="text-dark">Email : {this.state.data.email}</p>
                      ...
                      <div className="billing-content-sec">
                        ...
                        <div className="table-responsive">
                          <table className="table white-bg m-0">
                            <tbody>
                              ...
                              {this.state.loadingReferral == false ? (
                                <tr>
                                  <td>{t("referral_amount_applied")}</td>
                                  <td>
                                    {
                        ...
                        {freePayPerView == false ? (
                          <div className="mt-4">
                            ...
                            <form className="mt-3" action="" onChange={ this.handleChangePayment}>
